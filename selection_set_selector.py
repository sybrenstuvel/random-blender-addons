...
class POSE_OT_show_single_selection_set(Operator):
    bl_idname = 'post.show_single_selection_set'
    ...
    def execute(self, context):
        ...
        # Only show bones in the selection set
        for pose_bone in arm.pose.bones:
            pose_bone.bone.hide = pose_bone.name not in sel_set.bone_ids

            # Ensure that hidden bones are no longer selected.
...
class POSE_OT_show_single_selection_set_call_menu(Operator):
    bl_idname = 'pose.show_single_selection_set_call_menu'
    bl_label = 'Show Single Selection Set'
...
class POSE_MT_selection_sets(Menu):
    bl_label = POSE_OT_show_single_selection_set.bl_label
    poll = POSE_OT_show_single_selection_set.poll

    def draw(self, context):
        layout = self.layout
        layout.operator_context = 'EXEC_DEFAULT'
        for ss in context.object.selection_sets:
            props = layout.operator(POSE_OT_show_single_selection_set.bl_idname, text=ss.name)
...
def register():
    bpy.utils.register_class(POSE_OT_show_single_selection_set)
    bpy.utils.register_class(POSE_OT_show_single_selection_set_call_menu)
    bpy.utils.register_class(POSE_MT_selection_sets)

    wm = bpy.context.window_manager
    km = wm.keyconfigs.active.keymaps['Pose']

    kmi = km.keymap_items.new('wm.call_menu', 'W', 'PRESS', alt=True, ctrl=True)
...
def unregister():
    bpy.utils.unregister_class(POSE_OT_show_single_selection_set)
    bpy.utils.unregister_class(POSE_OT_show_single_selection_set_call_menu)
